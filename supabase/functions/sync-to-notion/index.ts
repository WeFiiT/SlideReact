...
function slideToProps(s: any) {
  return {
    ...
    'Discipline':             s.discipline        ? { select: { name: s.discipline        } } : { select: null },
    'Niveau Discipline (We.Horizon)': s.niveau_discipline ? { select: { name: s.niveau_discipline } } : { select: null },
    'Management':    s.management     ? { select: { name: s.management } }                                   : { select: null },
    'Sujets mission': { multi_select: (s.sujets_mission || []).map((n: string) => ({ name: n })) },
    'Outils':         { multi_select: (s.outils         || []).map((n: string) => ({ name: n })) },
    'Type produit':   { multi_select: (s.type_produit   || []).map((n: string) => ({ name: n })) },
    'Sharepoint URL': s.sharepoint_url ? { url: s.sharepoint_url } : { url: null },
    'Statut slide':  { select: { name: s.validated ? 'Ready' : 'Draft' } },
    'Slide ID':          text(s.id),
    'Titre':             text(s.titre),
